...
void InitSpawner()
{
    ...
    else //### Multi-Player and Skirmish Settings ###
    {
        ...
        if (NetUnitCount > 10) NetUnitCount = 10;
        NetTechLevel = SpawnIniGetInt("Settings", "TechLevel", 7);
        NetStartingCredits = SpawnIniGetInt("Settings", "Credits", 7000);
        if (NetStartingCredits == 0) NetStartingCredits = 1;
        if (NetStartingCredits > 40000) NetStartingCredits = 40000;
        ...
        for (int i = 0; i < 6; i++)
        {
            if (strlen(NetPlayers[i].name)) continue;
    ...
    else if (NetPlayerCount > 1) //Multi-Player
    {
        ...
        if (IsHost)
        {
            ...
            if (TunnelPort) InitTunnel();
            else InitHolePunching();
        }
        else WOL__StartGuestINetGame();
    }
    else //Skirmish
    {
        ...
        CUIManager__StartGame(CUIManagerObject);
    }
}
...
{
    if (!SpawnerActive) return IsLocalIp(Ip);
